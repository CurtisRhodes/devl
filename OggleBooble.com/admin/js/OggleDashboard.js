...
function performCreateNewFolder(newFolderName, sortOrder) {
    try {
        ...
        $('#dashBoardLoadingGif').hide();
        logAdminError("CAT", e, "perform create new folder");
    }
}

function logAdminError(errorCode, errorMessage, calledFrom) {
...
function showFiles() {
    try {
        ...
        $.ajax({
            ...
            error: function (jqXHR) {
                logAdminError("XHR", getXHRErrorDetails(jqXHR), "show files");
            }
        });
    } catch (e) {
        logAdminError("CAT", e, "show files");
    }
}
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (returnObject[0] == "ok") {
                    ...
                    logAdminError("AJX",data, "update folder counts");
                }
            },
            error: function (jqXHR) {
                $('#dashBoardLoadingGif').hide();
                logAdminError("XHR", getXHRErrorDetails(jqXHR), "update folder counts");
            }
        });
    } catch (e) {
        $('#dashBoardLoadingGif').hide();
        logAdminError("CAT",  e, "update folder counts");
    }
}
    ...
    function saveSortOrder() {
        try {
            ...
            $.ajax({
                ...
                error: function (jqXHR) {
                    logAdminError("AJX",  getXHRErrorDetails(jqXHR), "save sort order");
                }
            });
        } catch (e) {
            logAdminError("CAT", e, "save sort order");
        }
    }
}
...
// MOVE MANY
{
    ...
    function showMoveManyDialog(cx) {
        ...
        $.ajax({
            ...
            success: function (imgLinks) {
                ...
                if (imgLinks.indexOf("error") > -1)
                    ...
                    moveManyArray = [];
                    $.each(links, function (ndx, obj) {
                        $('#moveManyImageArea').append("<div class='sortBox'><img class='sortBoxImage' src='" + settingsImgRepo + "/" + obj.FileName + "'/>" +
        ...
        $('#txtMoveManySource').val($('#txtCurrentActiveFolder').val());
    }
...
function showAddStepChildDialog() {
